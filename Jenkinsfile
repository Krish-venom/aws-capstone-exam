...
stage('Terraform Init/Plan/Apply') {
  steps {
    dir('terraform') {
      // If you do NOT have an IAM role on the agent, uncomment withCredentials and create 'aws_creds' in Jenkins.
      // withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws_creds']]) {
        sh '''
          ...
          terraform -version
          uname -a || true
          ldd --version || true || echo "ldd not installed"
          env | grep -E 'HTTP_PROXY|HTTPS_PROXY|NO_PROXY|http_proxy|https_proxy|no_proxy' || true

          echo "=== Clean previous state of providers ==="
          rm -rf .terraform .terraform.lock.hcl || true

          echo "=== Init (upgrade, with retry) ==="
          (terraform init -input=false -upgrade) || (sleep 5 && terraform init -input=false -upgrade)
